...
class DeepQLearning:
    ...
    def __init__(self,env,gamma,epsilon,numberEpisodes):
        ...
        # state dimension
        self.stateDimension = 2* config.FEATURE_DIM
        # action dimension
        self.actionDimension= config.ACTION_DIM
    ...
    # @staticmethod
    def _get_input_of_net(self, state):
        # return (x_bar, mask)
        ip = np.concatenate((state[2], state[3]))
        return ip
    ...
    def createNetwork(self):
        model=Sequential()
        # model.add(Reshape(target_shape=(-1,self.stateDimension), input_shape=(config.FEATURE_DIM,2)))
    ...
    def selectAction(self,state,index_episode):
        import numpy as np
        
        # first index_episode episodes we select completely random actions to have enough exploration
        # change this
        if index_episode<1:
            return np.random.choice(config.ACTION_DIM)   
            
        # Returns a random real number in the half-open interval [0.0, 1.0)
        # this number is used for the epsilon greedy approach
        randomNumber=np.random.random()
        
        # after index_episode episodes, we slowly start to decrease the epsilon parameter
        ...
        # otherwise, we are selecting greedy actions
            ...
            # we return the index_episode where Qvalues[state,:] has the max value
            # that is, since the index_episode denotes an action, we select greedy actions
            
            ip_state = self._get_input_of_net(state).reshape(1,-1)
            Qvalues=self.mainNetwork.predict(ip_state)
          
            return np.random.choice(np.where(Qvalues[0,:]==np.max(Qvalues[0,:]))[0])
            # here we need to return the minimum index_episode since it can happen
            # that there are several identical maximal entries, for example 
            # import numpy as np
            # a=[0,1,1,0]
            # np.where(a==np.max(a))
            # this will return [1,2], but we only need a single index_episode
    ...
    def trainNetwork(self):
        ...
        if (len(self.replayBuffer)>self.batchReplayBufferSize):
            ...
            currentStateBatch=np.zeros(shape=(self.batchReplayBufferSize, 2*config.FEATURE_DIM))
            nextStateBatch=np.zeros(shape=(self.batchReplayBufferSize, 2*config.FEATURE_DIM))            
            # this will enumerate the tuple entries of the randomSampleBatch
            # index will loop through the number of tuples
            for index,state_tuple in enumerate(randomSampleBatch):
                # first entry of the tuple is the current state
                # (currentState,action,reward,nextState,terminalState)
                
                currentStateBatch[index,:] = self._get_input_of_net(state_tuple[0])
                # fourth entry of the tuple is the next state
                nextStateBatch[index,:] = self._get_input_of_net(state_tuple[3])
            ...
            # input for training
            input_batches=currentStateBatch
            # output for training
            output_batches=np.zeros(shape=(self.batchReplayBufferSize,self.ACTION_DIM))
            ...
            for index,(currentState,action,reward,nextState,terminated) in enumerate(randomSampleBatch):
                ...
                # if the next state if not the terminal state    
                    ...
                    y=reward + np.max(QnextStateTargetNetwork[index])
                
                # this is necessary for defining the cost function
                self.actionsAppend.append(action)
                
                # this actually does not matter since we do not use all the entries in the cost function
                output_batches[index]=QcurrentStateMainNetwork[index]
                # this is what matters
                output_batches[index,action]=y
            
            # here, we train the network
            self.mainNetwork.fit(input_batches,output_batches,batch_size = self.batchReplayBufferSize, verbose=0,epochs=100)     
...
if __name__ == "__main__":
    ...
    epsilon = 0.1
    n_episodes = 1000
    LearningQDeep=DeepQLearning(env,gamma,epsilon,n_episodes)
